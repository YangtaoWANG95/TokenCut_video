...
im_save_dir = '../FBMS/JPEGImages'
os.makedirs(im_save_dir, exist_ok=True)
anno_save_dir = '../FBMS/Annotations'
...
for vid in all_vids:
    ...
    start_idx = n_with_gt[0]
    save1 = os.path.join(im_save_dir, vid)
    os.makedirs(save1, exist_ok=True)
    print(f'video:{vid}, {n_with_gt}')
    for i, im in enumerate(images):
        img = os.path.join(vid_dir, im)
        save_name = str(i+start_idx).zfill(5) + ".jpg"
        print(f'video: {vid}, save: {save_name}')
