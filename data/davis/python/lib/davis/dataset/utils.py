...
def db_eval(techniques,sequences,inputdir=cfg.PATH.SEGMENTATION_DIR,metrics=None):
	...
	for technique in techniques:
		...
		log.info('Processing time: "%.3f"'%(time.time()-start))
...
def db_read_info():
	...
	print(f'file db_info: {cfg.FILES.DB_INFO}')
...
def db_read_sequences(eval_set):
	""" Read list of sequences. """
	if eval_set != 'all': 
		return [s for s in db_read_info().sequences if s.set==eval_set]
...
def db_read_eval(technique=None,measure=None,
		sequence=None,raw_eval=False,inputdir=cfg.PATH.EVAL_DIR, eval_set='all'):
	...
	sequences = _listify(sequence,
			lambda: [s.name for s in db_read_sequences(eval_set)])
...
def db_eval_view(db_eval_dict,technique,
	...
	db_sequences = db_read_sequences(eval_set)
