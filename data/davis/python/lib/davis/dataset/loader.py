...
class DAVISSegmentationLoader(object):
	...
	def __init__(self,cfg,sequence,masks_dir=None,ext_im="jpg",
		...
		masks_frames = map(lambda fn:
				int(os.path.splitext(os.path.basename(fn))[0]),self._masks.files);masks_frames=list(masks_frames)
		...
		image_frames = map(lambda fn:
				int(os.path.splitext(os.path.basename(fn))[0]),self._images.files);image_frames=list(image_frames)
	...
	def eval(self,db_segmentation,measure='J'):
		...
		if measure == 'J':
			...
			#raise Exception, "Unknown measure=[%s]. Valid options are measure={J,F,T}"%measure
                        print("Unknown measure=[%s]. Valid options are measure={J,F,T}"%measure)
